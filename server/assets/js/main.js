...
var author$project$Main$fetchSockets = elm$http$Http$get(
	{
		expect: A2(elm$http$Http$expectJson, author$project$Model$SocketsReceived, author$project$Main$socketsDecoder),
		url: '/Socket'
	});
var author$project$Model$Loading = {$: 'Loading'};
var author$project$Model$NewSocket = function (description) {
...
var author$project$Main$addSocket = function (socket) {
	return elm$http$Http$post(
		{
			...
			expect: A2(elm$http$Http$expectJson, author$project$Model$NewSocketAdded, author$project$Main$socketDecoder),
			url: '/Socket'
		});
};
var author$project$Model$SocketChanged = function (a) {
	return {$: 'SocketChanged', a: a};
};
...
var author$project$Main$changeSocket = function (socket) {
	return elm$http$Http$request(
		{
			body: elm$http$Http$multipartBody(
				_List_fromArray(
...
var author$project$Main$removeSocket = function (id) {
	return elm$http$Http$request(
		{
			body: elm$http$Http$emptyBody,
			expect: elm$http$Http$expectWhatever(author$project$Model$SocketDeleted),
			headers: _List_Nil,
			method: 'DELETE',
			timeout: elm$core$Maybe$Nothing,
			tracker: elm$core$Maybe$Nothing,
...
var author$project$Main$switchSocket = F2(
	function (state, socket) {
		...
		return elm$http$Http$post(
			{
				body: elm$http$Http$emptyBody,
				expect: elm$http$Http$expectWhatever(author$project$Model$SocketSwitched),
...
var author$project$Main$update = F2(
	function (msg, model) {
		switch (msg.$) {
			...
			case 'DeleteSocket':
				...
				if (_n1.$ === 'Sockets') {
					...
					return _Utils_Tuple2(
						...
						author$project$Main$removeSocket(id));
				} else {
			...
			case 'NewSocketDescriptionChanged':
				var desc = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							newSocket: author$project$Model$NewSocket(desc)
						}),
					elm$core$Platform$Cmd$none);
			case 'NewSocketAdd':
				var _n2 = model.sockets;
				switch (_n2.$) {
					case 'Loading':
						return _Utils_Tuple2(model, elm$core$Platform$Cmd$none);
					case 'Error':
					...
					default:
						...
						return _Utils_Tuple2(
							model,
							author$project$Main$addSocket(model.newSocket));
				}
			case 'NewSocketAdded':
				if (msg.a.$ === 'Ok') {
					var newSocket = msg.a.a;
					var _n3 = model.sockets;
					if (_n3.$ === 'Sockets') {
						var sockets = _n3.a;
						return _Utils_Tuple2(
							_Utils_update(
								...
								{
									sockets: author$project$Model$Sockets(
										A2(elm$core$List$cons, newSocket, sockets))
								}),
							elm$core$Platform$Cmd$none);
					} else {
						return _Utils_Tuple2(model, elm$core$Platform$Cmd$none);
			...
			case 'SocketDescriptionChanged':
				var id = msg.a;
				var desc = msg.b;
				var _n4 = model.sockets;
				if (_n4.$ === 'Sockets') {
					var sockets = _n4.a;
					var updateDesc = function (x) {
			...
			[
				...
				switch (_n0.$) {
					case 'Error':
						return A2(
							...
							_List_fromArray(
								[
									elm$html$Html$text('Error loading switches')
								]));
					case 'Loading':
						return A2(
							elm$html$Html$p,
							...
							_List_fromArray(
								[
									elm$html$Html$text('Loading switches')
								]));
					default:
						var sockets = _n0.a;
						return A2(
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								elm$html$Html$text('Add a switch')
							])),
						author$project$View$viewAddSwitch
					]))
			]),
		title: 'Switches'
	};
